...
$(document).ready(function(){
      ...
      switch (currentStage) {
        ...
        case "1":
          if (isNaN(donationAmount) || (donationAmount < 0.01)) {
            this.triggerAlert("Please choose a valid donation amount.",$('.donation-v2-amounts'));
            return false;
          } else {
            return true;
          }
        ...
        case "2":
          ...
          if (!validatedEmail) {
            ...
            this.triggerAlert("You must supply a valid email address",$('#donation_email'));
            return false;
          }
          if (!$('#donation_first_name').val()) {
            $('#donation_first_name').addClass('error');
            this.triggerAlert("You must supply a first name",$('#donation_first_name'));
            return false;
          }
          if (!$('#donation_last_name').val()) {
            $('#donation_last_name').addClass('error');
            this.triggerAlert("You must supply a last name",$('#donation_last_name'));
            return false;
          }
  ...
  if ($('.progress-indicator-stages').length) {
    ...
    $('.progress-stages .donation-v2-amounts .radio').not('[for="donation_amount_other"]').each(function(){$(this).click(function(event){
      StagedDonations.updateProgressIndicator(progressIndicator, 1);
      StagedDonations.updateProgressStages(progressStages, 1);
    });});
